...
export const LIABILITY_WAIVER = `RELEASE AND WAIVER OF LIABILITY
...
export const AGREEMENT_TYPES = {
  ...
  DAMAGE_FEES: 'damage_fees_acknowledgment',
} as const;
...
export const AGREEMENT_LABELS = {
  ...
  [AGREEMENT_TYPES.LIABILITY_WAIVER]: {
    label: 'Liability Waiver and Release',
    description: 'I have read and agree to the',
  },
  [AGREEMENT_TYPES.DAMAGE_FEES]: {
    label: 'Damage & Cleaning Fees',
    description: 'I understand and agree that I will be charged accordingly for any damages to equipment, simulators, screens, furniture, or property, as well as any extra cleaning fees as warranted. Charges will be applied to my payment method on file.',
  },
} as const;

export const POLICY_CONTENT: Record<string, { title: string; content: string }> = {
  [AGREEMENT_TYPES.TERMS_OF_SERVICE]: {
 ...
 * Used to create a tamper-proof fingerprint of the exact document
 * text the user agreed to at the time of acceptance.
 */
...
/**
 * Generate document hashes for all agreement types.
 * The damage_fees_acknowledgment uses the checkbox label text as its content.
 */
export async function getDocumentHashes(): Promise<Record<string, string>> {
  const damageFeeText = AGREEMENT_LABELS[AGREEMENT_TYPES.DAMAGE_FEES].description;

  const [tosHash, privacyHash, waiverHash, damageHash] = await Promise.all([
    sha256(TERMS_OF_SERVICE),
    sha256(PRIVACY_POLICY),
    sha256(LIABILITY_WAIVER),
    sha256(damageFeeText),
  ]);

  return {
    [AGREEMENT_TYPES.TERMS_OF_SERVICE]: tosHash,
    [AGREEMENT_TYPES.PRIVACY_POLICY]: privacyHash,
    [AGREEMENT_TYPES.LIABILITY_WAIVER]: waiverHash,
    [AGREEMENT_TYPES.DAMAGE_FEES]: damageHash,
  };
}
